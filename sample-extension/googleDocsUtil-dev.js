var googleDocsUtil = function() {
    ...
    function getGoogleDocument() {
        ...
        if (containsUserCaretDom()) {
            console.log("[DEBUG] conatainsUserCaretDom is true; so setting caret...")
            caret = getUserCaretDom();
            caretRect = caret.getBoundingClientRect();
        } else {
            console.log("[DEBUG] containsUserCaretDom is false...") ;
        }

        console.log("[DEBUG] paragraphrenderers.length: " + paragraphrenderers.length) ;
        console.log("[DEBUG] " + Object.getOwnPropertyNames(paragraphrenderers)) ;
        for (var i = 0; i < paragraphrenderers.length; i++) {
            var lineviews = paragraphrenderers[i].getElementsByClassName("kix-lineview");
            console.log("[DEBUG] using kix-selection-overlay to set select: " + lineviews) ;
            for (var j = 0; j < lineviews.length; j++) {
                ...
                for (var k = 0; k < wordhtmlgeneratorWordNodes.length; k++) {
                    ...
                    console.log("[DEBUG] - about to compute selection, selectionOverlays.length: " + selectionOverlays.length) ;
                    for (var l = 0; l < selectionOverlays.length; l++) {
                        ...
                        var selectionRect = selectionOverlay.getBoundingClientRect();
                            console.log("[DEBUG] setting selectedText to " + selectionStartIndex + "and " + selectionEndIndex ) ;
